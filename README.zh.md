...
打开 **设置 → 通用**。打开「液态玻璃」和／或「流体背景」。流体不依赖液态玻璃；两者都开时流体是壁纸。「交互动画」默认开启，给按钮弹窗和设置页 iOS 式缩放拉伸，不依赖液态玻璃；展开／收回／淡出滑块调节时长。

![通用设置：液态玻璃调参与流体背景](images/插件设置页1.png)

![通用设置：交互动画滑块](images/插件设置页2.png)
